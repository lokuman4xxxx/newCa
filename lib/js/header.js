function activateSubmenu(e) {
	...
	o.css({
		...
		left: n - 2,
		height: s
	}), i.find("a").addClass("maintainHover"), o.mouseenter(function() {
		i.find("a").addClass("maintainHover")
	})
}
...
function deactivateSubmenu(e) {
	var i = $(e),
		t = i.data("submenuId"),
		o = $("#" + t);
	o.css("display", "none"), i.find("a").removeClass("maintainHover")
}
...
function deactivateSubmenu(e) {
	...
	function i(i) {
		var t = e(this),
			...
			u = function() {
				r && clearTimeout(r), l.enter(this), k(this)
			},
			h = function() {
				l.exit(this)
			},
			...
			g = function() {
				...
				var r = t.offset(),
					...
					c = {
						x: r.left + t.outerWidth(),
						y: a.y
					},
					p = {
						x: r.left,
						y: r.top + t.outerHeight() + l.tolerance
					},
					u = {
						x: r.left + t.outerWidth(),
						y: p.y
				...
				var v = c,
					k = u;
				"left" == l.submenuDirection ? (v = p, k = a) : "below" == l.submenuDirection ? (v = u, k = p) : "above" == l.submenuDirection && (v = a, k = c);
				var g = i(h, v),
					y = i(h, k),
					w = i(f, v),
					m = i(f, k);
				return w > g && y > m ? (n = h, d) : (n = null, 0)
			};
		t.mouseleave(p).find(l.rowSelector).mouseenter(u).mouseleave(h).click(f), e(document).mousemove(c)
	}
	...
	var e = $(this).button("loading"),
		i = document.getElementById("termsAgreed").checked,
		t = -1 != window.location.href.indexOf("system") ? "mobileRegister.ohtml" : "system/mobileRegister.ohtml";
	$("#regPassword").val() != $("#regPasswordConfirm").val() ? (e.button("reset"), $("#error_regmsg").slideDown(200).text("Passwords do not match.")) : i ? $.ajax({
		type: "POST",
		url: t,
		data: {
			F_Tname: $("#regName").val(),
			Temail: $("#regEmail").val(),
			Tpwd1: $("#regPassword").val(),
			Tpwd2: $("#regPasswordConfirm").val()
		},
		success: function(i) {
			1 === i.exist && (e.button("reset"), $("#error_regmsg").text("Sorry, we cannot create an account using that email address. Please try another email address.").show()), 1 === i.success && window.location.reload()
		}
	}) : (e.button("reset"), $("#error_regmsg").text("Please agree the terms and conditions."))
}), $("#loginForm").submit(function(e) {
	e.preventDefault()
}), $("#userLog").click(function() {
	var e = $(this).button("loading"),
		i = -1 != window.location.href.indexOf("system") ? "mobileLogin.ohtml" : "system/mobileLogin.ohtml";
	$.ajax({
		type: "POST",
		url: i,
		data: {
			Temail: $("#logEmail").val(),
			Tpwd: $("#logPassword").val()
		},
		success: function(i) {
			0 === i.success && (e.button("reset"), $("#error_logmsg").text("Please check your information and try again.").show()), 1 === i.success && window.location.reload()
		}
	})
});
